...
def validate_common(name: str, resources: ResourceConfig) -> None:
...
def validate_required_paths(name: str, paths: list[Path]) -> None:
...
def validate_positional_args(step_name: str, positional_args: Any) -> None:
...
def validate_command_step(
    *,
    ...
    validate_common(name, resources)
    if not command:
...
def validate_simulation_step(
    *,
    ...
    validate_required_paths(name, paths)
...
def validate_pytest_step(
    *,
    ...
    if path is not None:
        ...
        validate_required_paths(name, [Path(p) for p in raw_paths])
...
def validate_python_step(
    *,
    ...
    if positional_args is not None:
        validate_positional_args(name, positional_args)
    ...
    validate_required_paths(name, [Path(path)])
...
def validate_notebook_step(
    *,
    ...
    validate_required_paths(name, [path])
