...
def calculate_keyspace(branches: list[dict[str, Any]]) -> dict[str, list[Any]]:
    ...
    for branch in branches[1:]:
        branch_dict = dict(collapse_nested_dict(branch))
        if set(branch_dict.keys()) != set(keyspace.keys()):
        ...
        for k, v in branch_dict.items():
            ...
            keyspace[k].add(v)
    result: dict[str, list[Any]] = {k: list(v) for k, v in keyspace.items()}
    return result
...
def expand_branch_templates(templates: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    final_branches = []
    for branch_config in expanded_branches:
        root: dict[str, Any] = {}
