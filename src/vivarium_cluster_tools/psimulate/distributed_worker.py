...
class ResilientWorker(Worker):
    ...
    def work(self, *args, **kwargs):
        ...
        logger.add(
            logging_directory / (str(worker_) + ".log"), level="DEBUG", serialize=True
        )
    ...
    def _monitor_work_horse_tick(self, job):
        ...
        if job_status not in [JobStatus.FINISHED, JobStatus.FAILED]:
            ...
            # Unhandled failure
            logger.error(
                f"(work-horse terminated unexpectedly; waitpid returned {ret_val})"
            )

            retry_handler(
                job,
                f"Work-horse process was terminated unexpectedly (waitpid returned {ret_val})",
                None,
                None,
            )
            ...
            if self.acceptable_failure_count < 0:
                logger.error(
                    "This worker has had multiple jobs fail. That may mean the host is sick. Terminating "
...
def worker(parameters: Mapping):
    ...
    job = (
        f"{os.environ['JOB_NAME']}: {os.environ['JOB_ID']}:{os.environ['SGE_TASK_ID']}"
    )
    ...
    try:
        ...
        if vct_globals.ARTIFACT_PATH_KEY in dict(branch_config).get(
            ...
            input_data_config.update(
                {
                    vct_globals.ARTIFACT_PATH_KEY: branch_config[
                        vct_globals.INPUT_DATA_KEY
...
def do_sim_epilogue(
    ...
    logger.info(
        f'Results reporting completed in {exec_time["results_minutes"]:.3f} minutes.'
    )
