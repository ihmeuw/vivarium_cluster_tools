            ...
            if artifact_path:
                ...
                if not Path(artifact_path).exists():
                    raise FileNotFoundError(
                        f"Cannot find artifact at path {artifact_path}"
                    )
...
class NativeSpecification:
    ...
    def get_valid_queues(max_runtime: str) -> List[str]:
        ...
        if len(runtime_args) != 3:
            raise ValueError(
                "Invalid --max-runtime supplied. Format should be hh:mm:ss."
            )
        else:
...
def concat_preserve_types(df_list: List[pd.DataFrame]) -> pd.DataFrame:
    ...
    columns_by_dtype = [
        list(dtype_group.index) for _, dtype_group in dtypes.groupby(dtypes)
    ]
...
def main(
    ...
    logger.info(
        "Starting jobs. Results will be written to: {}".format(ctx.output_directory)
    )
