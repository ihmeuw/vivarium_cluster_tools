...
def append_perf_data_to_central_logs(perf_df: pd.DataFrame, output_directory: Path) -> None:
    ...
    if artifact_path_col in perf_df.columns:
    ...
    perf_df = perf_df.drop(all_scenario_cols + ['compound_scenario'], axis=1)
    ...
    data_fits_in_file = (len(most_recent_file) + len(perf_df)) <= NUM_ROWS_PER_FILE

    if data_fits_in_file:
        first_file_with_data = most_recent_file
        #perf_df.to_csv(most_recent_file, mode='a', index=False)
    else:
        most_recent_index = int(Path(most_recent_file).stem.replace('log_summary_',''))
        new_index = most_recent_index + 1

        if len(most_recent_file) != NUM_ROWS_PER_FILE:
            first_file_with_data = most_recent_file
            rows_to_append = NUM_ROWS_PER_FILE - len(perf_df)
            #perf_df[:rows_to_append].to_csv(most_recent_file, mode='a', index=False)
            perf_df = perf_df[rows_to_append:]
        else:
            first_file_with_data = Path(performance_logs_dir) / f'log_summary_{new_index}.csv'

        num_appends = math.ceil(len(perf_df) / NUM_ROWS_PER_FILE)

        for append_num in range(num_appends):
            new_file = Path(performance_logs_dir) / f'log_summary_{new_index}.csv'
