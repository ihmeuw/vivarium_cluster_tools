...
def log_path():
    return Path(
        "/mnt/team/simulation_science/pub/models/project_name/results/model_version/artifact/YYYY_MM_DD_HH_MM_SS/logs/yyyy_mm_dd_hh_mm_ss_runtype/worker_logs"
    )
...
def test_data_parsing(df_name, log_path, tmp_path, request):
    ...
    assert runner_data["original_log_file_path"].squeeze() == str(
        Path(log_path) / "log_summary.csv"
    )


def test_valid_log_path(log_path, artifact_perf_df, caplog, tmp_path, monkeypatch):
    monkeypatch.setattr(
        "vivarium_cluster_tools.psimulate.runner.CENTRAL_PERFORMANCE_LOGS_DIRECTORY", tmp_path
    )
    monkeypatch.setattr(
        "vivarium_cluster_tools.psimulate.runner.NUM_ROWS_PER_CENTRAL_LOG_FILE", 4
    )
...
@pytest.mark.parametrize(
    ...
    [
        Path(
            "/mnt/team/simulation_science/pub/models/project_name/model_version/artifact/YYYY_MM_DD_HH_MM_SS/logs/yyyy_mm_dd_hh_mm_ss_runtype/worker_logs"
        ),
        Path(
            "/mnt/team/simulation_science/pub/project_and_model_version/artifact/YYYY_MM_DD_HH_MM_SS/logs/yyyy_mm_dd_hh_mm_ss_runtype/worker_logs"
        ),
        Path(
            "/ihme/homes/user/run/artifact/YYYY_MM_DD_HH_MM_SS/logs/yyyy_mm_dd_hh_mm_ss_runtype/worker_logs"
        ),
    ],
)
...
def test_appending(
    ...
    MAX_NUM_ROWS = 4
    monkeypatch.setattr(
        "vivarium_cluster_tools.psimulate.runner.NUM_ROWS_PER_CENTRAL_LOG_FILE", MAX_NUM_ROWS
    )
    monkeypatch.setattr(
        "vivarium_cluster_tools.psimulate.runner.CENTRAL_PERFORMANCE_LOGS_DIRECTORY", tmp_path
    )
