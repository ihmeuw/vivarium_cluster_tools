...
def hostname_init():
    return {
        "good": ["user@int-uge-archive-p006",],
        "bad": ["user@gen-uge-submit-p01", "-submit-", "user@gen-uge-submit-p02",],
    }
...
def test_mkdir_set_permissions(permissions_params: List) -> None:
    ...
    permissions: Optional[str] = permissions_params[1] if permissions_params[
        1
    ] else "drwxrwxr-x"
